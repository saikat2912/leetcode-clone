    ...
    return(
        <tbody className="text-white">
            {problems.map((doc,idx) =>{
                const difficultyColor= doc.difficulty ==="Easy" ?"text-dark-green-s":doc.difficulty==="Medium"?"text-dark-yellow-500":"text-dark-pink";
                return(
                    <tr className={`${idx %2 ===1 ?"bg-dark-layer-1":""}`} key={doc.id}>
                        ...
                        <td className={'px-6 py-4'}>
                            {doc.videoId ? (<AiFillYoutube fontSize={"28"}  onClick={()=>setYoutubePlayer({isOpen:true,videoId:doc.videoId as string})}/>):(<p className="text-gray-400">Coming Soon</p>)}
                        </td>
                    </tr>
                )
            })}
